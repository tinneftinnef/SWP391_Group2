...
<html lang="en">
    ...
    <section id="billboard" class="position-relative overflow-hidden bg-light-blue p-t-75">
        <form id="paymentForm" action="online-pay" class="bg0 p-t-75 p-b-85" method="post">
            <div class="container">
                <div class="row">
                                                ...
                                                <tr class="table_row">
                                                    ...
                                                    <td class="column-4">
                                                        <div class="wrap-num-product flex-w m-l-auto m-r-0">
                                                            <a onclick="down(${product.productId})" class="btn-num-product-down cl8 hov-btn3 trans-04 flex-c-m">
                                                                <i class="fs-16 zmdi zmdi-minus"></i>
                                                            </a>
                                                            <input onChange="checkInput(this)" min="1" max="${product.stock}" id="quantity-${cartItem.productId}" class="mtext-104 cl3 txt-center num-product" type="number" name="newQuantity" value="${cartItem.quantity}">

                                                            <a onclick="up(${product.productId})" class="btn-num-product-up cl8 hov-btn3 trans-04 flex-c-m">
                                                                <i class="fs-16 zmdi zmdi-plus"></i>
                                                            </a>
                                                        </div>
                                                    </td>
                                                    <td class="column-5">${product.price * cartItem.quantity} VND</td>
                                                    ...
                                                    <td class="column-5">
                                                        <a onClick="DeleteCart(${cartItem.id})" class="btn btn-outline-dark" style="margin-right: 2rem">
                                                            Delete
                                                        </a></td>
                                                </tr>
                                            </c:forEach>
                    ...
                    <div class="col-sm-10 col-lg-7 col-xl-5 m-lr-auto m-b-50">
                        <div class="bor10 p-lr-40 p-t-30 p-b-40 m-l-63 m-r-40 m-lr-0-xl p-lr-15-sm">
                            ...
                            <c:if test="${carts.size() > 0}">
                                ...
                                <a onclick="submitForm()" class="flex-c-m stext-101 cl0 size-116 bg3 bor14 hov-btn3 p-lr-15 trans-04 pointer btn">
                                    Online PAY
                                </a>
                                
                                <a href="checkout?action=cash" class="flex-c-m stext-101 cl0 size-116 bg3 bor14 hov-btn3 p-lr-15 trans-04 pointer btn">
                                    Order now
                                </a>
                            </c:if>
                        </div>
    ...
    <script>
        function checkInput(input) {
            ...
            if (isNaN(newValue) || newValue > maxValue) {
                showError("Invalid input. Please enter a number less than or equal to " + maxValue + " and greater 0");
                input.value = maxValue;
            } else if (newValue <= 0) {
                showError("Invalid input. Please enter a number greater 0");
                input.value = 1;
            }
        }
        const up = (id) => {
            const idGet = 'quantity-' + id;
            var result = document.getElementById(idGet);
            var sst = parseInt(result.value);
            if (sst >= parseInt(result.max)) {
                showError("Quantity limit reached")
                return false;
            }
            if (!isNaN(sst)) {
                result.value++;
        ...
        function updateCart(productId, quantity, result) {
            ...
            var url = "http://localhost:8080/Tien/cart?action=update&productId=" + productId + "&quantity=" + quantity;
                ...
                if (xhr.readyState == 4 && xhr.status == 200) {
                    if (xhr.status == 200) {
                        ...
                        if (response == "success") {
                            ...
                            showError("Quantity out of stock");
                        } else {
        ...
        function DeleteCart(cartId) {
            ...
            var xhr = new XMLHttpRequest();
            var url = "http://localhost:8080/Tien/cart?action=delete&cartId=" + cartId;

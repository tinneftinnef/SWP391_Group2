...
<html>
                            ...
                            <div class="wrap-table-shopping-cart">
                                <table class="table-shopping-cart">
                                    ...
                                    <c:forEach var="cartItem" items="${orderDetails}">
                                        ...
                                        <tr class="table_row">
                                            ...
                                            <td class="column-2">
                                                ...
                                                <span style="font-size: 12px;">(Seri: ${cartItem.serialNumber})</span>
                                            </td>
                                            <td class="column-3"> <fmt:formatNumber value="${product.price}" type="number" maxFractionDigits="0" />₫</td>
                                            <td class="column-2" style="text-align: right">
                                                <input min="1" max="${product.stock}" id="quantity-${cartItem.productId}" class="mtext-104 cl3 txt-center num-product" type="number" name="newQuantity" value="${cartItem.quantity}">
                                            </td>
                                            <td class="column-5">
                                    ...
                                    <c:if test="${carts.size() == 0}">
                                        <tr>
                                            <td colspan="10" style="text-align: center; padding: 10px 0;">
                                                NO PRODUCT
                                            </td>
                                        </tr>
                                    </c:if>
                            ...
                            <div class="flex-w flex-t p-t-27 p-b-33">
                                ...
                                <div style="width: 100%" class="mt-3">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control" name="name" placeholder="Full name" value="${order.name}" readonly/>
                                    </div>
                                    <div class="form-group">
                                        <label>
                                            Phone:  
                                        </label>
                                        <input class="form-control" name="phone" placeholder="Phone" value="${order.phone}" readonly/>
                                    </div>
                                    <div class="form-group">
                                        <label>
                                            Address 
                                        </label>
                                        <input class="form-control" name="address" placeholder="Address" value="${order.address}" readonly/>
                                    </div>
                                    <div class="form-group">
                                        <label>
                                            Note:  
                                        </label>
                                        <input class="form-control" name="note" placeholder="Note" value="${order.note}" readonly/>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <c:if test="${order.status == 3}">
                                            <div style="padding: 10px;">
                                                ...
                                                <a onClick="ConfirmOrder(${order.orderId})" class="btn btn-success">Confirm order</a>
                                            </div>
                                        </c:if>
                                        <c:if test="${order.status == 4}">
                                            ...
                                            <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#warrantyModal" 
                                                    data-order-id="${order.orderId}" data-product-id="${orderDetail.product_id}" data-serial-number="${orderDetail.serial_number}">
                                                Request Warranty
                                            </button>
                                        </c:if>
                                    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="warrantyModalLabel">Request Warranty</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="warrantyForm" method="post" action="warranty" enctype="multipart/form-data">
                        <input type="hidden" id="orderId" name="orderId" value="${order.orderId}">
                        <div class="form-group">
                            <label for="serialNumber">Serial Number</label>
                            <input class="form-control" type="text" id="serialNumber" name="serialNumber" required>
                        </div>
                        <div class="form-group">
    ...
    <c:if test="${param.warranty != null && param.type != null && param.type> 0}">
        <script>
            Swal.fire({
                icon: 'success',
                title: 'Successfully',
                text: "Warranty successfully"
            });
        </script>
    </c:if>
    <c:if test="${param.warranty != null && param.type != null && param.type == 0}">
    ...
    <script>
            function ConfirmOrder(orderId) {
                if (confirm('Are you sure to confirm this order?') == false) {
                    return;
                }
                var xhr = new XMLHttpRequest();
                var url = "<%=request.getContextPath()%>/order-history?action=confirm&orderId=" + orderId;
                    ...
                    if (xhr.readyState == 4 && xhr.status == 200) {
                        if (xhr.status == 200) {
                            ...
                            if (response == "success") {
                                ...
                                setTimeout(function () {
                                    window.location.href = "<%=request.getContextPath()%>/order-history?action=view&orderId=" + orderId;
                                }, 200);
                            } else
                            if (response == "auth") {
                            ...
                            if (response == "date") {
                                showError("Status order can not confirm.");
                                setTimeout(function () {
                                    window.location.href = "<%=request.getContextPath()%>/order-history?action=view&orderId=" + orderId;
                                }, 200);
                            } else {
                                showError("Confirm fail.");
                            }
                        } else {
                            showError("Confirm fail.");
                        }
    ...
    <script>
            ...
            if (newSearchParams.toString()) {
                newURL += '?action=view&' + newSearchParams.toString();
            }
